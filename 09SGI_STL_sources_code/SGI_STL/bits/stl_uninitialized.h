...
// peimin: 这里的3个函数对应copy fill fill_n函数作用于未初始化的内存上
namespace std
{

  // uninitialized_copy
 // peimin: 这里的构造规则是要么全ok要么一个不
  ...
  */
  ...
  // peimin: char wchat_t的特化
  inline char*
  uninitialized_copy(const char* __first, const char* __last, char* __result)
  {
    memmove(__result, __first, __last - __first); // pemin: memmove 直接调用
