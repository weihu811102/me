...
// peimin:  一级配置器
  template <int __inst>
    ...
    {
    private:
		// peimin: 这些函数用来处理内存不足的情况
      static void* _S_oom_malloc(size_t);
      static void* _S_oom_realloc(void*, size_t);
      static void (* __malloc_alloc_oom_handler)(); // peimin: 这是一个函数指针
	...
	// peimin: 调用malloc 来分配内存 分配不到的时候该用_S_oom_malloc
	...
	// peimin: 使用realloc来分配内存失败时用_S_oom_realloc处理
	if (0 == __result) __result = _S_oom_realloc(__p, __new_sz);
	return __result;
      }

	// peimin: 仿真C++的set_new_handler 可以通过它指定自己的out-of-memory hander函数
...
// peimin: __simple_alloc是包装后的接口 一般都用它
...
// 二级内存分配器__default_alloc_template
template<bool __threads, int __inst>
  class __default_alloc_template
  {
  private:
    enum {_ALIGN = 8};				// peimin: 对其数
    enum {_MAX_BYTES = 128};
    ...
    static char* 		_S_start_free; // peimin: free lists的开始
    static char* 		_S_end_free;   // peimin: 结束
    static size_t 		_S_heap_size;
    
    static _STL_mutex_lock 	_S_node_allocator_lock;

    static size_t
    _S_round_up(size_t __bytes) // peimin: 对齐到8
  ...
  public:
    // __n must be > 0
  // peimin: 内存分配函数大于128bytes调用第一级配置器小于调用free lists
  // peimin: 如果fill lists可用块 直接用 没有上调到8的倍数调用refill重新填充
    static void* 
    allocate(size_t __n)
    {
      void* __ret = 0;
      ...
      else 
	{
	  ...
	  if (__result == 0)
	    __ret = _S_refill(_S_round_up(__n)); // peimin: 没有可用的
	  else 
	    {
    ...
    // __p may not be 0
    // peimin: 大于128用第一配置器释放否则寻找对应的free lists回收
    static void 
    deallocate(void* __p, size_t __n)
    {
      if (__n > (size_t) _MAX_BYTES)
  ...
  template<bool __threads, int __inst>
								...
								int& __nobjs)
    {
      ...
      if (__bytes_left >= __total_bytes) // peimin: 剩余空间够
      {
        __result = _S_start_free;
        _S_start_free += __total_bytes;
        return(__result);
      } 
      else if (__bytes_left >= __size) // peimin: 够一个块
	{
      ...
      else // peimin: 连一个块都不够
	{
	  ...
	  if (0 == _S_start_free) // peimin: 堆空间不够
	    {
	      size_t __i;
	      _Obj* volatile* __my_free_list;
	      _Obj* __p;
