...
// peimin: vector 内存分配的基类
template <class _Tp, class _Allocator, bool _IsStatic>
class _Vector_alloc_base {
public:
  typedef typename _Alloc_traits<_Tp, _Allocator>::allocator_type // peimin: simple_alloc默认调用的
          allocator_type;
...
protected:
  allocator_type _M_data_allocator;  // peimin: 数据内存分配器
  _Tp* _M_start;					  //  peimin: 起始位置	
  _Tp* _M_finish;                    // peimin: 已经使用的
  _Tp* _M_end_of_storage;           // peimin: 末尾
...
*/
template <class _Tp, class _Alloc = allocator<_Tp> > // peimin: 默认使用allocator分配内存
class vector : protected _Vector_base<_Tp, _Alloc> // peimin: 这里使用了父类中变量
{
...
public:
	// peimin: vector的嵌套型别定义
  typedef _Tp 						value_type;
  typedef value_type* 					pointer; // peimin: 指针就是原生指针
...
protected:
	// peimin: 容器的区间指针
  using _Base::_M_allocate;        // peimin: 这里使用了父类中的变量是内存分配器
  using _Base::_M_deallocate;
  using _Base::_M_start;           // peimin: 这块存储空间的起始位置
  using _Base::_M_finish;          // peimin: 被使用的范围
  using _Base::_M_end_of_storage; // peimin: 这块存储空间的尾端
...
// peimin: 构造函数
  ...
  */
   ...
   *  vector.
  */
  reference back() { return *(end() - 1); } // peimin: 前开后壁的区间[}
  ...
  */
  ...
  push_back()
  {
    if (_M_finish != _M_end_of_storage) {
      ...
      ++_M_finish; // peimin: 插入新元素调整水位
    }
    else
      _M_insert_aux(end()); // peimin: 剩余空间不够了
  }
#endif
  ...
  */
  ...
  insert(iterator __position, const _Tp& __x)
  {
    ...
    return begin() + __n; // peimin: 返回的是新的便宜位置
  }
...
protected:
  ...
  template <class _ForwardIterator>
  	// peimin: 分配大小为n的空间且拷贝first->last的数据
...
vector<_Tp,_Alloc>::operator=(const vector<_Tp, _Alloc>& __x)
{
  if (&__x != this) { // peimin: 避免自身复制
    const size_type __xlen = __x.size();
    if (__xlen > capacity()) { // peimin: 大于容量重新分配并且拷贝数据
    ...
    else if (size() >= __xlen) { // peimin: 现在大小够用直接覆盖原空间的数据
      iterator __i(copy(__x.begin(), __x.end(), begin()));
      _Destroy(__i, end()); // peimin: free 剩余的空间 copy函数返回的是当前迭代器的移动位置
    }
    else {
      copy(__x.begin(), __x.begin() + size(), _M_start); // peimin: 这个位置拷贝到_M_finish
      uninitialized_copy(__x.begin() + size(), __x.end(), _M_finish); // peimin: 这部分是未出世的拷贝
    }
    _M_finish = _M_start + __xlen; // peimin: 移动水位
  }
  return *this;
}
