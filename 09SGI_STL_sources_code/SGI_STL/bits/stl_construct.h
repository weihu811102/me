...
// peimin: 这里使用triat 关于这个参考<C++ 模板> 一书
  ...
  template <class _ForwardIterator> 
    inline void
    __destroy_aux(_ForwardIterator, _ForwardIterator, __true_type) // 有一些析构函数时不用调用的
    { }
...
// peimin:	此函数设法找出元素的数值类别进而利用__type_traits求取最适当的措施求职
// peimin: 这里使用了trait 手法 这个以后再介绍
  template <class _ForwardIterator>
    inline void
